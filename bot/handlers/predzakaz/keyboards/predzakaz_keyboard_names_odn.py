...
def predzakaz_keyboard_names(folder, number):
    ...
    for i, item in enumerate(folder[number-10:number+1]):
        with open(f"/Users/sayzyyy/PycharmProjects/VapeBot/bot/handlers/predzakaz/keyboards/parsers/odnorazki/{item}") as f:
            data = json.load(f)
        ...
        if i == 10 and number == 10:
            ...
            builder.add(InlineKeyboardButton(
                ...
                len(os.listdir("/Users/sayzyyy/PycharmProjects/VapeBot/bot/handlers/predzakaz/keyboards/parsers/odnorazki")) > number):
            builder.add(InlineKeyboardButton(
                text="Назад <-",
