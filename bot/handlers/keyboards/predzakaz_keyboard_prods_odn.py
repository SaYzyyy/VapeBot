...
def show_prods(file, number_o):
    ...
    if os.path.exists(f"../parsers/odnorazki/{file.replace("(", "(_").replace(")", "_)").replace("_", " ")}0 MG ).json"):
        with open(f"../parsers/odnorazki/{file.replace("(", "(_").replace(")", "_)").replace("_", " ")}0 MG ).json", "r", encoding="utf-8") as f:
            data = json.load(f)
