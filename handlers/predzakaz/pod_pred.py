...
from handlers.start import number_pod, number_pod_t, string_for_admins, pod_type
...
async def pods(callback_query: types.CallbackQuery):
    ...
    global number_pod_t
    global pod_type
    await callback_query.message.answer(text="Выберите название под-системы:", reply_markup=show_prods_pods("../parsers/new_category2.json", number = 10))
...
async def next_pod(callback_query: types.CallbackQuery):
    ...
    await callback_query.message.answer(text="Выбеите производителя:", reply_markup=show_prods_pods(
        "../parsers/new_category2.json", number = number_pod))
    await callback_query.answer()
    await callback_query.message.delete()
